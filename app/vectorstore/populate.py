...
def populate_database() -> None:
    ...
    # 2. Limpa coleção anterior se --clear for informado
    clear_existing = "--clear" in sys.argv
    if clear_existing:
        print("🧹 Limpando dados antigos da coleção (solicitado --clear)...")
        try:
            ...
            print(f"⚠️ Erro ao limpar a coleção: {str(e)}")
    else:
        print("ℹ️ Iniciando em modo de recuperação/resume. Os chunks já indexados serão preservados.")
    ...
    # Gera IDs deterministas para todos os chunks
    all_ids = [f"{doc.metadata['source_title']}_{doc.metadata['chunk_index']}" for doc in all_documents]
    
    # Se não formos limpar, verifica quais chunks já existem na base através dos metadados
    existing_ids = set()
    if not clear_existing:
        print("🔍 Verificando chunks já existentes no ChromaDB (via metadados)...")
        try:
            res = vectorstore.get(include=["metadatas"])
            if res and "metadatas" in res and res["metadatas"]:
                for meta in res["metadatas"]:
                    if meta and "source_title" in meta and "chunk_index" in meta:
                        existing_ids.add(f"{meta['source_title']}_{meta['chunk_index']}")
            print(f"ℹ️ Encontrados {len(existing_ids)} chunks já indexados de {len(all_ids)} no total.")
        except Exception as e:
            print(f"⚠️ Erro ao verificar metadados existentes: {e}")
            
    # Filtra os chunks que precisam ser indexados
    docs_to_index = []
    ids_to_index = []
    for doc, doc_id in zip(all_documents, all_ids):
    ...
    if not docs_to_index:
        print("====================================================")
        print("✨ Todos os chunks já estão indexados no ChromaDB!")
        print("====================================================")
        return
        
    print(f"🚀 Enviando {len(docs_to_index)} chunks novos para o ChromaDB e gerando embeddings...")
    ...
    try:
        ...
        batch_size = 20
        print(f"🚀 Enviando {len(docs_to_index)} chunks novos para o ChromaDB em lotes de {batch_size}...")
        ...
        for i in range(0, len(docs_to_index), batch_size):
            ...
            print(f"📦 Indexando lote {i // batch_size + 1} / {(len(docs_to_index) - 1) // batch_size + 1} ({len(batch_docs)} chunks)...")
            ...
            while retries > 0 and not success:
                try:
                    ...
                    if "RESOURCE_EXHAUSTED" in err_msg or "429" in err_msg:
                        ...
                        print(f"⚠️ Erro de conexão/temporário durante indexação: {err_msg}. Aguardando 15 segundos... (Tentativas restantes: {retries})")
                        time.sleep(15)
            if not success:
                raise Exception("Lote falhou após esgotar tentativas de reenvio devido a limites de taxa ou conexão.")

        print("====================================================")
        print(f"✨ Sucesso! Todos os novos chunks foram indexados com sucesso!")
